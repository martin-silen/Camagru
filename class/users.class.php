...
class Users {
  ...
  private function checkPassword() {
    ...
    if ($this->passwd != $this->passwdVerif)
      return $this->message = "The passwords are not identical.";
    if (!preg_match('/(?=.*[0-9])(?=.*[A-Za-z]).{7,30}/', $this->passwd))
      return $this->message = "Your password must be composed of at least 7 characters, including a letter and a number.";
  }
  ...
  private function checkNewUser() {
    ...
    if (filter_var($this->email, FILTER_VALIDATE_EMAIL) === false)
      return $this->message = "Your email is invalid.";
  }
  ...
  public function confirmUser() {
    try {
      ...
      $user = $req->fetch(PDO::FETCH_ASSOC);
      if (!$user)
        return $this->message = "Your account has already been validated or the link has expired.";
      $req = $this->db->prepare("UPDATE `users` SET `confirm` = ?, `token` = ?, `token_expires` = ? WHERE `token` = ?");
      $req->execute(array(1, NULL, NULL, $this->token));
      $this->message = "Your account has been successfully confirmed. Welcome " . $user['login'] . " !";
    }
    catch (Exception $e) {
      die('Error : ' . $e->getMessage());
    }
  ...
  public function connectUser() {
    try {
      $user = $this->getUser();
      if (!$user)
        return $this->message = "The username entered does not belong to any account.";
      if ($user['confirm'] == 0)
        return $this->message = "Your account has not yet been validated.<br /> Please follow the link received by email.";
  ...
  public function resetPassword() {
    try {
      ...
      $user = $req->fetch(PDO::FETCH_ASSOC);
      if (!$user)
        return $this->message = "The link has expired or was not followed correctly.";
      self::checkPassword();
      if ($this->message != null)
        return ;
